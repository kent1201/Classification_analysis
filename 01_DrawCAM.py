...
from utils import ProcessConfig, GetTransform, GetDataPath, GetModel, CheckSavePath, Onnx2Pytorch, GetConfigs, Str2List, GetCudaDevice, Onnx2Torch
...
def draw_CAM(model, model_ft, img, img_path, verbose=False, save_path=None):
    ...
    # (14, 768)
    fc_weights = model.state_dict()['Gemm_6691.weight'].cpu().numpy()
    ...
    def __returnCAM(feature_conv, weight_softmax, class_idx):
        """
        Params:
            feature_conv (numpy): 特徵層輸出
            weight_softmax (numpy): fully connection layer 的權重
            class_idx (list): 根據類別機率由高至低排序的類別 index
        Return:
            output_cam (list of numpy): 
        """
...
if __name__=='__main__':
    ...
    configs = GetConfigs(r"D:\Users\kentTsai\Documents\Kinsus project\data_analysis\ModelB.ini")
    ...
    try:
        ...
        print("No .pth found.")
        pth_model_path = Onnx2Torch(model_path)
